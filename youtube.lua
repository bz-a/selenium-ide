...
**
** JSLover'z version of YouTube.lua
**
**   Updated: Fri, Jan 8, 2016 --- 1/8/16, 10:19:19am EST
**  Location: JSLover.secsrv.net/VLC/playlist/youtube.lua
**
** Modified from VLC's Git version: http://git.videolan.org/?p=vlc.git;a=history;f=share/lua/playlist/youtube.lua
**                      Commit msg: youtube.lua: resolve XML entities in Art URL
**                      Commit URL: http://git.videolan.org/?p=vlc.git;a=commit;h=8cd7277cde3d929f7baa0db61ce6162a9848b25e
**                         Updated: Sat, Dec 19, 2015 --- 12/19/15, 12:08:24pm EST
**                            Hash: 8cd7277cde3d929f7baa0db61ce6162a9848b25e
**
        ...
        if not fmt or not itag or tonumber( itag ) == tonumber( fmt ) then
            ...
            if url then
                ...
                if not sig then
                    ...
                    if sig then
                        vlc.msg.info( "Found "..string.len( sig ).."-character scrambled signature for youtube video URL, attempting to descramble... " )
                        if js_url then
...
function parse()
    ...
    then -- This is the HTML page's URL
        ...
        luaPattern_title_nc="<meta +name=\"title\" +content=\"(.-)\""
        luaPattern_title_cn="<meta +content=\"(.-)\" +name=\"title\""
        ...
        while true do
            ...
            if not title then
				title=string.match(line, luaPattern_title_nc)
			end

            if not title then
				title=string.match(line, luaPattern_title_cn)
			end
            ...
            if string.match( line, "<meta property=\"og:image\"" ) then
				...
				if arturl then
					vlc.msg.info("youtube.lua: arturl --".." ("..arturl..") -- IF")
				else
					vlc.msg.info("youtube.lua: arturl --".." ("..""..") -- ELSE")
				end
            end

            -- Comment out VLC code...
			...
			-- 	if string.match(line, "<meta itemprop=\"datePublished\"") then
			-- 		datePublished=string.match(line, "content=\"(.-)\"")
			-- 		if datePublished then
			-- 			-- title = 'dP('..datePublished..') - '..title
			-- 		else
			-- 			-- title = 'dP(nope!) - '..title
			-- 		end
			-- 	end
			...
			--  Get Video Published Date as String
			-- Example Lines...
			--    <div id="watch-uploader-info"><strong class="watch-time-text">Published on Sep 5, 2015</strong></div><div
				if string.match(line, "watch%-time%-text") then
					datePublishedStr=string.match(line, ">Published on (.-)<")
					if datePublishedStr then
						-- title = 'dP('..datePublished..') - '..title
						title = datePublishedStr..' - '..title
					else
						-- title = 'dP(nope!) - '..title
					end
				end
			...
			--  Get YouTube Username (if available)
            if not yt_username then
				yt_username=string.match(line, "<link +itemprop=\"url\" +href=\"http://www.youtube.com/user/([^\"]*)\"")
			end
			-- /Get YouTube Username (if available)

			--  Get YouTube Channel ID/Name
            if not yt_chan_name then
                if string.match(line, "yt%-user%-info") then
					-- vlc.msg.info("youtube.lua: yt-user-info -- line: ("..line..")")
					line = vlc.readline()
					-- vlc.msg.info("youtube.lua: yt-user-info -- line: ("..line..")")

					yt_chan_id, yt_chan_name = string.match(line, "data%-ytid=\"(.-)\"[^>]*>([^<]*)<")
            ...
            if not artist and yt_chan_name then
				if yt_chan_name==yt_username then
					artist = yt_username
				elseif yt_username then
					artist = yt_chan_name.." ("..yt_username..")"
				elseif yt_chan_id then
					artist = yt_chan_name.." ("..yt_chan_id..")"
				else
					artist = yt_chan_name
				end

				if not yt_chan_name then yt_chan_name='<null>' end
				if not yt_username then yt_username='<null>' end
				if not yt_chan_id then yt_chan_id='<null>' end
				vlc.msg.info("youtube.lua: artist info --".." cn("..yt_chan_name..")".." un("..yt_username..")".." id("..yt_chan_id..")")
        ...
        if not path then
            ...
            if video_id then
                ...
                -- Update: el=detailpage fails for some age-restricted videos (example: https://www.youtube.com/watch?v=2KApp699WdE),
                -- using el=embedded might be better, but it might fail for videos with embedding disabled, must test.
                path = vlc.access.."://www.youtube.com/get_video_info?video_id="..video_id..format.."&el=embedded"
